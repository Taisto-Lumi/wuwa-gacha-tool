...
| 方式 | 适用场景 | 说明 |
| --- | --- | --- |
| Client.log 日志文件 | 本机安装鸣潮 | 选择游戏根目录下的 `Client\Saved\Logs\Client.log`，工具读取日志并提取链接同步 |
...
- 配置并校验 `Client.log` 文件路径，保存后直接读取该日志扫描
...
- 抽卡数据、导入状态和卡池边界保存在可同步的 `gacha-data.db`；日志路径与资源缓存保存在设备专属的 `app-state.db`
...
- Client.log 扫描模式仅读取日志以提取官方抽卡链接，不读取游戏账号密码
...
- 使用 Client.log 扫描前，请先在游戏内打开一次唤取记录页面，否则日志中可能没有可用链接
